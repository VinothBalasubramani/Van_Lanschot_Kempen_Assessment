...
variable "tags"            { type = map(string) default = { env = "dev", owner = "strada" } }
...
module "adf" {
  ...
  sql_database_name         = "dst_dev_sqldb"
  databricks_workspace_url  = "https://adb-1111111111111111.10.azuredatabricks.net"
}

module "dbx_uc" {
  source            = "../../modules/databricks"
  workspace_id      = "1111111111111111"
  catalog_name      = "team_dev"
  create_metastore  = true
  metastore_name    = "uc-dev-metastore"
  storage_root      = "abfss://uc-dev@${module.storage.account_name}.dfs.core.windows.net/"
  dev_group_name    = "sg-dbx-developer-dev"
  admin_group_name  = "sg-dbx-admin-dev"
}
