...
variable "env"             { type = string  default = "prd" }
variable "tags"            { type = map(string) default = { env = "prd", owner = "strada" } }
...
provider "databricks" {
  host = "https://adb-3333333333333333.10.azuredatabricks.net"
}

resource "azurerm_resource_group" "rg" {
  name     = "rg-strada-downstream-prd"
...
module "adf" {
  ...
  sql_database_name         = "dst_prd_sqldb"
  databricks_workspace_url  = "https://adb-3333333333333333.10.azuredatabricks.net"
}

module "dbx_uc" {
  source            = "../../modules/databricks"
  workspace_id      = "3333333333333333"
  catalog_name      = "team_prd"
  create_metastore  = true
  metastore_name    = "uc-prd-metastore"
  storage_root      = "abfss://uc-prd@${module.storage.account_name}.dfs.core.windows.net/"
  dev_group_name    = "sg-dbx-developer-prd"
  admin_group_name  = "sg-dbx-admin-prd"
}
